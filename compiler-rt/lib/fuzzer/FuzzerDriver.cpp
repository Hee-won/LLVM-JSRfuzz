...
int FuzzerDriver(int *argc, char ***argv, UserCallback Callback) {
  ...
  std::string Argv0((*argv)[0]); // 첫 번째 인수(argv[0])를 문자열로 변환해 Argv0에 저장. 보통 실행 파일의 경로가 저장됨.
  EF = new ExternalFunctions(); // 외부 함수들을 담은 객체를 생성.
  if (EF->LLVMFuzzerInitialize) // 퍼징 전에 초기 설정을 적용하는 함수
  ...
  ParseFlags(Args, EF); // 전달된 명령줄 인수를 파싱하여 퍼징에 사용할 플래그(옵션)들을 설정.
  if (Flags.help) {
    PrintHelp();
    return 0;
  }
