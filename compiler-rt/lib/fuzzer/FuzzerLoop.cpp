...
bool Fuzzer::RunOne(const uint8_t *Data, size_t Size, bool MayDeleteFile,
  ...
  TPC.CollectFeatures([&](uint32_t Feature) { // 커버리지 정보를 모아 변형에 활용
  ...
  if (FoundUniqFeatures) // 새로운 유일한 기능의 수를 FoundUniqFeatures 포인터에 저장
  ...
  // 만약 기존 Corpus에 있던 입력과 비교하여 새로운 기능이 더 작고 유용하면, 기존 입력을 더 작은 입력으로 교체
...
ATTRIBUTE_NOINLINE bool Fuzzer::ExecuteCallback(const uint8_t *Data,
  ...
  CurrentUnitSize = Size; // Size는 입력 데이터의 크기이며, 이를 CurrentUnitSize에 저장.
  int CBRes = 0; // 콜백 함수의 결과값을 저장할 변수 CBRes를 선언하고 초기화.
  { // 이 블록 안에서는 콜백 함수(CB)를 실행하고 그 결과를 CBRes에 저장함.
